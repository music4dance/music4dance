...
| `/api/suggestion/*`, `/api/song/?filter=...`, `/api/search?search=...` returning 400 for ordinary free text (song titles, CJK search phrases, multi-word phrases) | **Suspected internal bug — open, Sep 14 2026** | Same "400 with no matching `LogWarning`" signature as the already-open `/api/usagelog/batch` investigation (see [[client-side-usage-logging]] §10.1), but on plain `axios.get` calls, not `sendBeacon` — rules out the beacon-specific theory as the sole cause. Leading hypothesis: `[ValidateAntiForgeryToken]` failing because the anonymous HTML page that minted the embedded `xsrfToken`/cookie pair was served from the 5-minute Front Door/browser cache (`Cache-Control: public, max-age=300`, `M4dApplicationExtensions.cs:826`), so the token baked into a cached page can outlive or mismatch the real antiforgery cookie. `SuggestionController` dominates the count simply because it fires on every keystroke. Not yet confirmed | Investigate — instrument `SuggestionController` with the same explicit-validate-and-log pattern as `UsageLogController.LogBatch` to confirm |
| `/api/suggestion/https:/open.spotify.com/track/...` (single slash, occasionally two track IDs concatenated together) | Minor — needs investigation | `SuggestionEntry.vue`'s free-text `watch` fires whenever a Spotify track URL ends up in the search box (drag/paste path via `useDropTarget`/`checkServiceAndWarn`); `encodeURIComponent` on the client is correct, so the collapse from `https://` to `https:/` happens downstream (framework/proxy path normalization collapsing `//`). `AzureSuggestions` (`SongIndex.cs:1426`) then throws on the colon/slash-laden query and is caught internally, returning `NotFound` — so these are always 404, not 400, and mostly harmless, but a full spotify URL was never a sensible thing to feed a free-text suggester in the first place | Low priority — worth a client-side guard to skip the suggestion fetch entirely when the field looks like a URL |
...
| `/vclient/assets/*.js`, `/vclient/assets/*.css` (Vite content-hashed filenames) | **Needs re-verification** | Previously documented (row above) as "all hits clustered in a ~12 min window matching a deploy." The Sep 14 2026 CSV contradicts that: hits for distinct hashed filenames are spread across more than 24 hours (`2026-09-13T00:07` through `2026-09-14T17:04`), well outside any single 5-minute cache window — exactly the "watch for hits outside a post-deploy window" case the original entry called out. Could be multiple deploys across the window, long-lived browser tabs doing a lazy-loaded chunk fetch long after the tab was opened, or a real caching problem | Investigate — correlate hit timestamps against actual deploy timestamps for this window before concluding either way |
