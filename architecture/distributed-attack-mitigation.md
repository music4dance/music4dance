...
| `/song/album` (62 hits, no `title`), `/song/artist` + `/song/artist/` (56 hits, no `name`) | Known bot pattern | Extends the Aug-16 `/song/artist?name` row: the bare, query-less forms are now the highest-volume non-attack app URLs. `SongController.Album`/`Artist` 404 with "Empty album title not valid" / "Empty artist name not valid". Real links always carry a value - the client builds them in `AlbumList.vue` and `ArtistNames.ts` | No action on the 404 itself. Optional UX win: redirect the bare forms to `/song/artists` (which now exists) instead of erroring |
