...
| `/api/suggestion/*`, `/api/song/?filter=...`, `/api/search?search=...` returning 400 for ordinary free text (song titles, CJK search phrases, multi-word phrases) | **Internal bug — root-caused Sep 14 2026** | Same "400 with no matching `LogWarning`" signature as the already-open `/api/usagelog/batch` investigation (see [[client-side-usage-logging]] §10.1), on plain `axios.get` calls, not just `sendBeacon`. Root cause confirmed from production logs: the antiforgery cookie defaults to session-lifetime while the anonymous HTML page that embeds its matching token is browser-cached for 5 minutes (`Cache-Control: public, max-age=300`) — a browser restart within that window leaves a cached page's embedded token with no matching cookie. (Not a CDN issue: Azure Front Door was never actually provisioned — see [[front-door-implementation]].) `SuggestionController` dominates the count simply because it fires on every keystroke | Fixed 2026-09-14: `SuggestionController` instrumented with the same explicit-validate-and-log pattern as `UsageLogController.LogBatch`; antiforgery cookie given a 1-day `Expiration` in `M4dApplicationExtensions.cs` to outlive a browser restart. Not yet verified in production |
| `/api/suggestion/https:/open.spotify.com/track/...` (single slash, occasionally two track IDs concatenated together) | Fixed 2026-09-14 | `SuggestionEntry.vue` had two independent, uncoordinated watchers on the same input: the free-text suggestion `watch` (fires `/api/suggestion/{value}` on every keystroke ≥2 chars) and `@input="checkServiceAndWarn(...)"` (the actual Spotify/Apple Music id/URL handling, via `ServiceMatcher`/`/api/servicetrack/...`). Both ran regardless of what the other found, so pasting a Spotify link fired a pointless suggestion-endpoint fetch alongside the real lookup. (The single-slash collapse itself — `https://` becoming `https:/` — is a separate, harmless framework/proxy path-normalization quirk; `AzureSuggestions` threw on the resulting query and was caught internally, returning 404, not 400.) | Fixed: `SuggestionEntry.vue`'s suggestion `watch` now skips the fetch when `ServiceMatcher.match()` recognizes the input as a service id/URL, since `checkServiceAndWarn` already owns that case |
| `/content/Beat-*.png`, `/content/Energy-*.png`, `/content/Mood-*.png`, `/Content/tools.png`, `/Content/info.png`, `/Content/ballroom.png` | Benign — stale external links | These images lived at `DanceCalc/m4d/Content/*.png` before the 2016 MVC-era layout (commit `f8bd1c84`); the current site serves the same icon set from `/images/icons/*.png` via the Vite-bundled client (`SongTable.vue`). Nothing in the current codebase references `/content/...` anymore — these are old bookmarks/search-cache/hotlinks from a decade-old URL scheme, not a live bug | Ignore (optional: 301-redirect `/content/*.png` → `/images/icons/*.png` if this becomes high-volume) |
| `/vclient/assets/*.js`, `/vclient/assets/*.css` (Vite content-hashed filenames) | **Needs re-verification — deploy theory ruled out** | Previously documented (row above) as "all hits clustered in a ~12 min window matching a deploy." The Sep 14 2026 CSV contradicts that outright: hits for distinct hashed filenames span `2026-09-13T00:07` through `2026-09-14T17:04`, but `az webapp deployment list` / Kudu's `deployments/` history shows **no deploy to `msc4dnc` at all in that window** — the last one before it was 2026-09-10T03:53. So this can't be post-deploy staleness this time; leading candidates are long-lived/suspended browser tabs doing a lazy-loaded chunk fetch from a session opened before the *previous* deploy, or the anonymous-page browser cache (see the antiforgery row below and [[client-side-usage-logging]] §10.1 — same `Cache-Control: public, max-age=300` mechanism, no CDN involved since Front Door was never provisioned) serving pre-Sep-10 HTML far past its 5-minute intended window | Investigate — would need real browser telemetry (tab-open duration, cache hit/miss) to pin down further; not code-fixable without more data |
| `/llms.txt` | Benign platform probe (AI crawler) | AI-agent/LLM crawlers requesting a machine-readable site summary (analogous to `robots.txt`/`sitemap.xml`), a convention that didn't exist when the `sitemap.xml` row above was written | Ignore (optional: publish one) |
| `/ip`, `/mcp`, `/sse` | Bad actor / scanner (probable) | Generic probes for exposed debug (`/ip`), Model Context Protocol, or server-sent-events endpoints — a newer scanning pattern looking for exposed AI-agent backdoors, not specific to this app | Ignore — not added to `IsKnownAttackUrl` since these are short, generic path segments a legitimate future feature could plausibly use; revisit if volume grows |
| `/proc/self/environ`, `/proc/self/cmdline`, `/.git/config`, `/.git-credentials`, `/.aws/credentials`, `/.aws/config`, `/.npmrc`, `/.s3cfg`, `/.boto`, `/credentials.json`, `/service-account.json`, `/firebase-adminsdk.json`, `/terraform.tfstate`, `/var/run/secrets/kubernetes.io/serviceaccount/token`, `*/editor/filemanager/browser/default/browser.html` (FCKeditor, tried under ~20 decoy prefixes), `/graphql`, `/*/.env` (prefixed variants like `/admin/.env`, `/wordpress/.env`) | Bad actor / scanner | Generic secret/credential-hunting and CMS file-manager exploit probes, same family as the pre-existing `/wp-admin/`/`/.env` rows above but previously missed by the narrower prefix-only matcher | Fixed 2026-09-14: added to `Http4xxTracker.IsKnownAttackUrl`'s substring list (see above) |
| `/song/home/privacypolicy`, `/dances/privacypolicy`, `/song/details/home/privacypolicy`, `/users/info/home/privacypolicy`, ... | **Internal bug - fixed Sep 19, 2026** | The cookie-consent banner's "learn more" link in `_head.cshtml` was configured with a *document-relative* href, `"home/privacypolicy#GDPR"`. A browser resolves that against the current page, so the link landed on `/song/home/privacypolicy` from a song page, `/dances/home/privacypolicy` from a dance page, and so on - it only ever worked from the site root. ~30 hits across eight distinct prefixes in the Sep 19 CSV, each one a real visitor who clicked through to a 404 from a GDPR banner | Fixed: href is now root-relative (`/home/privacypolicy#GDPR`) |
| `/cdnjs.cloudflare.com/ajax/libs/cookieconsent2/...`, `/pagead2.googlesyndication.com/pagead/js/adsbygoogle.js`, `/favicon.ico/cdnjs.cloudflare.com/...` | **Internal bug - fixed Sep 19, 2026** | `_head.cshtml` loaded three third-party assets over *protocol-relative* URLs (`//host/path`), a pre-HTTPS-everywhere idiom. Browsers resolve those correctly, but a number of crawlers and embedded HTTP clients treat the leading `//` as a path and request it from this origin - hence the host name showing up as a path segment, sometimes nested under whatever page they were on | Fixed: cdnjs, Google Fonts and adsbygoogle now load over explicit `https://` |
| `/favicon.ico` (172 hits - highest-volume single URL in the Sep 19 CSV), `/favicon.png`, `/assets/favicon.ico`, `/img/favicon.ico`, `/static/favicon.ico` | **Internal gap - fixed Sep 19, 2026** | `_head.cshtml` linked `~/images/favicon.png` and nothing was ever served at the site root. Browsers, crawlers and link-preview bots request `/favicon.ico` unconditionally, whether or not the page declares an icon, so this 404ed on a large share of all traffic | Fixed: `wwwroot/favicon.ico` added (the existing 48x48 `images/favicon.png` wrapped in an ICO container - no new artwork), and `_head.cshtml` now declares both. The prefixed variants (`/assets/`, `/img/`, `/static/`) are scanner guesses and stay 404 |
| `/apple-touch-icon.png`, `/apple-touch-icon-precomposed.png`, `/apple-touch-icon-120x120*.png` (~284 hits) | Benign platform probe | Row above from Aug 16 still applies; volume is now second only to `/favicon.ico`. Fixing it properly needs a real 180x180 icon, which the 48x48 source cannot be upscaled into without looking bad | Open (cosmetic): needs artwork, not code |
| `/.ssh/id_rsa`, `/.ssh/authorized_keys`, `/server.key`, `/key.pem`, `/docker-compose.yml`, `/Dockerfile`, `/appsettings.Production.json`, `/local.settings.json`, `/serverless.yaml`, `/.svn/entries`, `/.vscode/sftp.json`, `/.idea/WebServers.xml`, `/.gitlab-ci.yml`, `/.claude/settings.local.json`, `/.htpasswd`, `/.bashrc`, `/rclone.conf`, `/dump.sql.gz`, `/db.tar.gz`, `/storage/logs/laravel.log`, `/secrets.env`, `/serviceAccountKey.json`, `/__/firebase/init.json` | Bad actor / scanner | A broad secret-scanning sweep on Sep 17: SSH and TLS key material, CI/CD config, container manifests, framework settings files, database dumps and backup archives. Same family as the Sep 14 additions, just a wider dictionary. Note `/secrets.env` and `/config/env/aws_credentials.env` - a bare `.env` *suffix*, which the old `/.env` substring (directory-prefixed only) missed | Fixed 2026-09-19: added to `IsKnownAttackUrl` (substring `.env` now matches the suffix form too) |
| `/cgi-bin/authLogin.cgi`, `*/server/php/` (jQuery-File-Upload, ~7 decoy prefixes), `/Telerik.Web.UI.WebResource.axd?type=rau`, `/sitecore/shell/sitecore.version.xml`, `/solr/`, `/owa/`, `/modules/mod_webshell/`, `/blocks/rce/lang/en/`, `/admin/controller/extension/extension/`, `/sites/default/files/`, `/media/system/js/core.js`, `/sugar_version.json`, `/showLogin.cc`, `/.tmb/`, `/userfiles?path=../../../.env` | Bad actor / scanner | Appliance- and CMS-specific exploit paths (QNAP, Sangfor, Telerik deserialization, Joomla/Drupal/Moodle/OpenCart/SugarCRM/OJS) plus file-manager path traversal | Fixed 2026-09-19: added to `IsKnownAttackUrl` |
| `/z9x8c7v6b5-debug-trigger-music4dance.net`, `/api/fs/exec`, `/api/inngest`, `/inngest`, `/api/designer/v1/file-content`, `/api/templates/preview`, `/read-document`, `/.vite/manifest.json`, `/dist/manifest.json`, `/build/manifest.json`, `/webpack-stats.json`, `/asset-manifest.json`, `/__vite_rsc_findSourceMapURL?filename=file:///app/.env` | Bad actor / scanner | A single coordinated sweep at 2026-09-17T06:17 (20 hits each, seconds apart) probing for exposed AI-app and dev-tooling backdoors: host-specific debug triggers, arbitrary-exec endpoints, workflow runners, and bundler metadata that leaks source paths. The `__vite_rsc_findSourceMapURL` variant is a genuine arbitrary-file-read attempt dressed as a source-map lookup (`file:///app/.env`, `file:///root/.ssh/id_rsa`, `file:///proc/self/environ`) | Fixed 2026-09-19: added to `IsKnownAttackUrl`. The generic name `/manifest.json` was deliberately *not* added - a future PWA manifest would use it |
| `/login`, `/signin`, `/signup`, `/register`, `/dashboard`, `/panel`, `/portal`, `/secure`, `/app`, `/account`, `/backoffice`, `/console`, `/forgot-password`, `/reset-password`, `/admin/login`, `/user/login`, `/api`, ... (exactly 20 hits each, one burst) | Bad actor / scanner | Auth-path dictionary sweep, same burst as the row above. The uniform count and sub-second spacing are the signature | **Not** added to `IsKnownAttackUrl` - these are short, generic segments a real feature could use, and filtering them would hide a genuinely broken internal link. Recognize the burst signature instead |
| `/old/`, `/new/`, `/backup/`, `/test/`, `/bak`, `/bk`, `/www`, `/site`, `/sitio`, `/sito`, `/demo`, `/2017`...`/2026` (each with `/Old`, `/OLD` case variants) | Bad actor / scanner | Classic backup-directory and stale-copy sweep; the ALL-CAPS/Titlecase variants of the same word are the tell | Not added - same reasoning as the auth-path row |
| `/.well-known/assetlinks.json`, `/.well-known/apple-app-site-association`, `/.well-known/openid-configuration`, `/.well-known/security.txt`, `/.well-known/gpc.json`, `/.well-known/nodeinfo`, `/.well-known/trust.txt`, `/security.txt`, `/trust.txt`, `/ai.txt`, `/app-ads.txt` | Benign platform probe | Auto-discovery by mobile OSes, identity clients, AI crawlers and ad verifiers. Extends the existing `/.well-known/traffic-advice` row | Ignore (optional: a `/.well-known/security.txt` is cheap and conventional) |
| `/sitemap_index.xml`, `/post-sitemap.xml`, `/page-sitemap.xml`, `/sitemap-news.xml`, `/sitemap.xml.gz`, `/sitemap1.xml`, `/sitemaps/sitemap.xml`, ... | Benign platform probe (crawler) | Crawlers walking the WordPress sitemap-naming conventions after the plain `/sitemap.xml` 404ed. All of it goes away if we ever publish a sitemap | Ignore - same disposition as the `/sitemap.xml` row |
| `/song/artist` + `/song/artist/` (56 hits, no `name`) | Known bot pattern - **redirected Sep 22, 2026** | Extends the Aug-16 `/song/artist?name` row: the bare, query-less forms were the highest-volume non-attack app URLs. Real links always carry a value, so these are crawlers truncating the query | Fixed 2026-09-22 (#284): `SongController.Artist` now 301s a nameless request to `/song/artists`, the browsable index added in #278, so a crawler learns the bare form has a permanent home. Gated on the `ArtistIndex` flag - with it off, `Artists()` 404s anyway, so the original error stands. Client link builders unified on `ArtistNames.artistPageUrl` at the same time, so the site no longer emits both the slashed and unslashed shapes |
| `/song/album` (62 hits, no `title`) | Known bot pattern | Same crawler behavior as the artist row above; `SongController.Album` 404s with "Empty album title not valid" and real links (built in `AlbumList.vue`) always carry a title | No action - deliberately left alone in #284. There is no album index to redirect to, and a 404 is the honest answer to a request with no album in it |
| `/dancehttps:/www.music4dance.net/dances/jump-swings/west-coast-swing` | External - fabricated link | `jump-swings` appears nowhere in this codebase, and the path is `"/dance"` concatenated with a full absolute URL. Signature of a hallucinated or string-mangled link from an external referrer, not something we emit | Ignore |
| `/dances/samba&ved=2ahUKEw...`, `/dances/cha-cha&ved=...` | External - mangled referrer | Google result URLs whose `?ved=` tracking parameter was rewritten to `&ved=` by some intermediary, turning it into part of the path | Ignore |
| `/api/health/status` returning **405** | Benign - wrong verb | The only 405 in the set. An external monitor is probing the health endpoint with a method it does not accept | Ignore unless it grows |
