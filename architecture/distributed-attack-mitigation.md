...
turned up dozens of previously-unclassified secret/credential-scanning and CMS-exploit probes;
current substring list: `.php`, `/.env`, `/.git`, `/.aws/`, `/.npmrc`, `/.s3cfg`, `/.boto`,
`/proc/self/`, `editor/filemanager/browser/default/browser.html` (the FCKeditor/CKEditor
file-manager exploit, probed under a couple dozen decoy prefixes — `/admin/`, `/fckeditor/`,
`/manager/`, `/ckeditor/`, etc.), `kubernetes.io/serviceaccount`, `/graphql`,
`service-account.json`, `firebase-adminsdk`, `credentials.json`, `terraform.tfstate`.
