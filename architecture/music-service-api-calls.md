...
**Failure path:** neither `GetMusicServiceResults` nor `MusicServiceAction` call `EnsureSuccessStatusCode`. On any status other than 200 or 429 (`GetMusicServiceResults` also now handles 401 — see below), `responseString` is never set, so the code falls into `if (responseString == null) throw new WebException(response.ReasonPhrase)`. This constructed `WebException` has no `Response` object, so the `catch (WebException we)` block's `we.Response is HttpWebResponse r` check is always false and the exception is simply rethrown. `MusicServiceAction` (the write path) still has no branch that inspects a 401/403 from the data endpoint itself and retries — left alone deliberately, since blindly retrying a non-idempotent POST (create playlist, add track) on a stale-token 401 risks a duplicate write. A dead *refresh* token on the write path is still caught one layer up, in `AdmAuthentication.GetServiceAuthorization` (called to build the `Authorization` header before the request is even sent) — see [§ Spotify Refresh-Token Expiration Handling](#spotify-refresh-token-expiration-handling).

**`GetMusicServiceResults` 401 handling (read path only):** a data-API 401 (`WWW-Authenticate: ... error="invalid_token"`) is treated separately from the generic failure path — before falling into the `responseString == null` branch, a `response.StatusCode == HttpStatusCode.Unauthorized` check (bounded to 2 retries via a local `authRetries` counter) calls `AdmAuthentication.InvalidateServiceAuthorization(configuration, service.Id, principal)` and `continue`s the retry loop. This covers the case where a token was valid by our own bookkeeping (cached, not yet past its locally-tracked expiry) but Spotify rejects it anyway — observed in practice on `BatchISRC`/`BatchEchoNest`-style long-running batches using the **app-level** client-credentials token (`GetISRCData`/`GetMusicServiceTrack` are called with no `principal`, so `AdmAuthentication.SetupService` resolves to the static `s_spotify` singleton, not a user's `SpotUserAuthentication`). Without this, nothing ever cleared the cached app-level `Token` except its own internal expiry `Timer`, so once Spotify rejected it early, every remaining item in the batch kept resending the same dead token — each failure caught and logged individually by `SongController.BatchProcess`'s per-item `catch (Exception e)`, so the batch appeared to silently stop making progress rather than aborting outright. Retries are bounded because a persistently-rejected token (e.g. genuinely bad credentials) should still surface as an error rather than loop.
...
A narrower alternative to evicting the whole cached instance is `AdmAuthentication.InvalidateServiceAuthorization(configuration, serviceType, principal?)` — resolves the same instance `SetupService` would (app-level `s_spotify` singleton if `principal` is null/unauthenticated, otherwise the cached `s_users` entry) and calls its `InvalidateToken()`, which nulls `Token` and disposes the pending renewal `Timer` without discarding the instance (so a `SpotUserAuthentication`'s `RefreshToken` is preserved — the next `GetAccessToken()` call transparently re-derives an access token from it). Used by `GetMusicServiceResults` to recover from a data-API 401 — see [§ HTTP Layer](#http-layer).
...
- **`GET CreateSpotify`**: calls `_spotifyAuthService.CheckSpotifyAccess(User, authResult)` (not the plain `CanSpotify` boolean) so it can tell apart "never connected" from "connected before, but Spotify just rejected a refresh" (`WasRejected`). If the user is authenticated but not currently Spotify-authorized and *does* have a Spotify login on file (`HasSpotifyLogin`), redirects to `GetSpotifyOAuthRedirectUrl(returnUrl, expired: spotifyAuthRejected)` — a `?reason=expired` query param on the redirect makes the shared Login page (see below) show a distinct message for the rejected case. Otherwise renders the `SpotifyCreateInfo` form (title/description/count/filter) with `CanSpotify`/`IsPremium`/`SubscriptionLevel` flags for the view to gate on.
- **`POST CreateSpotify`**: re-checks `canSpotify`; if false, shows the "Connect your account to Spotify" info view (no redirect — this is a form submission, not a fresh page load). If true:
  ...
  3. A `SpotifyAuthExpiredException` (thrown if a token refresh gets rejected mid-creation) is caught ahead of the blanket `catch (Exception e)` and shown as a distinct "reconnect your Spotify account" `Info` view with a link built from `GetSpotifyOAuthRedirectUrl`; any other exception still falls through to the generic `"Unable to create a Spotify playlist at this time. Please report the issue. ({e.Message})"` error view. See [§ Spotify Refresh-Token Expiration Handling](#spotify-refresh-token-expiration-handling) for the full mechanism.

### Shared Login Page "reason=expired" Messaging

`GetSpotifyOAuthRedirectUrl` builds a URL to `/Identity/Account/Login?provider=Spotify&returnUrl=...`, optionally appending `&reason=expired`. `Login.cshtml.cs`'s `OnGetAsync` reads that as a new `reason` query param and exposes it as `Model.Reason`; `Login.cshtml` branches on `Model.Reason == "expired"` (alongside the existing `Model.Provider == "Spotify"` check) to show "your Spotify sign-in has expired or was revoked, please reconnect" instead of the generic "you aren't signed in through Spotify yet" copy — both point at the same "Sign in with Spotify" button. `SongController.CreateSpotify` GET passes `expired: true` here; `SpotifyPlaylistController`'s Vue-facing path (below) surfaces the same distinction via a `reauthRequired` JSON field instead, since it has no server-rendered page to redirect to.

### `SpotifyPlaylistController` (`m4d/APIControllers/SpotifyPlaylistController.cs`) — Vue "add to playlist" widget

- **`GET api/spotify/playlist/user`**: `_spotifyAuthService.ValidateSpotifyAccess` (authenticated + premium + Spotify OAuth) then `MusicServiceManager.GetUserPlaylists` — `GET /v1/me/playlists` (paginated via the response's `next` URL). Validation failures map to 401/402/403 via `HandleValidationError`; any other exception is caught and returns a generic 500.
...
- `ValidateSpotifyAccess` calls `CheckSpotifyAccess` (not the plain `CanSpotify` boolean), so its `NoSpotifyOAuth` result carries `ReauthRequired` when the "no access" outcome was Spotify rejecting a refresh rather than the user never connecting. `HandleValidationError`'s `NoSpotifyOAuth` branch includes `reauthRequired` in the 403 body alongside `message`/`connectUrl`. A rejection can still occur *after* this preflight passes (e.g. the token expires between the check and the real write) — that path throws `SpotifyAuthExpiredException` from the actual `AddTrackToPlaylist`/`GetUserPlaylists` call, caught separately (see below) with the same `reauthRequired: true` shape. Any other exception still falls into the controller's blanket `catch (Exception ex)`, logged and returned as a generic 500.
...
- `AdmAuthentication.GetServiceAuthorization` and `CheckAccess` (the shared implementation behind `HasAccess`) both catch `SpotifyAuthExpiredException` and call `EvictUser` to remove the poisoned entry from `s_users`, so a subsequent request (in particular, after the user reconnects Spotify and gets a fresh cookie) re-runs `TryCreate` from scratch instead of returning the dead cached instance.
  - `HasAccess`/`CheckAccess` (used by `CanSpotify`/`CheckSpotifyAccess`/`ValidateSpotifyAccess` preflight checks) swallow the exception and report "no access" rather than propagating it — these are advisory checks, not the API call itself. `HasAccess` just returns `false` (unchanged signature, used where callers don't need to know why); `CheckAccess` additionally returns `WasRejected: true` so a caller can distinguish "never connected" from "was connected, Spotify rejected the refresh" and show a different message (see `SongController.CreateSpotify` GET and the Login-page `reason=expired` messaging below).
...
- The 403 response body's `reauthRequired: true` flag (set for both the `SpotifyAuthExpiredException` catches and the `ValidateSpotifyAccess`/`NoSpotifyOAuth(reauthRequired: true)` preflight case — absent/falsy for the plain "never connected" case) is read in `handleError` and stored in a `spotifyReauthRequired` ref, passed to `SpotifyRequirementsModal` as `:reauth-required`.
- `SpotifyRequirementsModal` shows an `alert-warning` banner ("Your Spotify connection has expired...") and swaps the CTA button/next-step copy from "Connect Spotify Account" to "Reconnect Spotify Account" when `reauthRequired` is true. The CTA's link also changes: `!reauthRequired` still points at `/identity/account/manage/externallogins` (lets the user link Spotify for the first time), but `reauthRequired` points at `/Identity/Account/Login?provider=Spotify&returnUrl=...&reason=expired` instead — the same Login-page flow used by the MVC path. This matters because "manage external logins" only lets you add a login that isn't linked yet; for a rejected refresh token the Spotify login *is* already linked (just its tokens are dead), so that page shows Spotify as already connected with no way to actually refresh it — only re-running the OAuth challenge via the sign-in page (which re-authorizes the existing linked login and captures fresh tokens) fixes it.
