...
1. Re-fetches all songs for the user using the same `SearchAll` + `WasEditedBy` approach.
2. Calls `Song.AdminModify(properties, dms)` on each matched song.
3. Bulk-updates the Azure index once after all songs are processed.
...
var songs = allSongs.Where(s => s.WasEditedBy(userName, capturedFrom, capturedTo)).ToList();
```
...
Both the GET and POST controllers rely on `SongIndex.SearchAll`, which is a thin wrapper over
`SongIndex.StreamAll`. `StreamAll` pages through Azure Search in 1000-song batches (the API
maximum), yielding songs one by one to keep peak memory bounded to a single page rather than
