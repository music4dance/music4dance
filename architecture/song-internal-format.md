...
- The two on-disk snapshot files that package many of these logs together (§12)
...
- **[contributor-test-environments.md](contributor-test-environments.md)** — the sandbox host that
...
## 12. Related On-Disk Snapshot Formats

Two files package many of the per-song logs described above into one document. Both were read and
written by a 2026-08-23 one-off migration (`local/scripts/backfill-test-isrc.js`, not committed —
`local/` is gitignored) that backfilled ISRC data into the older test fixture; the format
differences below are exactly what that script had to reconcile, and are the reason this section
exists.

### 12.1 Contributor Test Fixture (`m4dModels.Sandbox/TestData/dancestatistics.txt`)

This file is the embedded, already-public, PII-cleaned dataset behind the no-external-service
sandbox host (`m4d.Sandbox`) and the L0/L1 contributor rungs — see
[contributor-test-environments.md](contributor-test-environments.md). It is committed to the repo
and deserializes as a `DanceStatsInstance` (`m4dModels/DanceStatsInstance.cs`, `[JsonConstructor]`),
via Newtonsoft.Json, camelCased:

```json
{
  "dances": [ /* DanceStats — per-dance song counts, tag summaries, top song IDs, etc. */ ],
  "groups": [ /* DanceStats — dance-family groupings (e.g. all Waltzes) */ ],
  "cachedSongs": [ /* string[] — one full serialized song log per entry, see below */ ],
  "tagGroups": [ /* TagGroup — the site-wide tag vocabulary */ ]
}
```

**`cachedSongs`** is the field this section is about: a JSON array of strings, each one a complete
song produced by `Song.ToString()` (via `SongCache.Serialize()`, `m4dModels/SongCache.cs`) — i.e.
exactly the §1.1 wire format, `SongId={guid}\t<tab-delimited properties>`. Real per-song edit
history, not synthetic data: titles, artists, tempos, tags, dance ratings, purchase IDs, and user
attribution (with usernames replaced by pseudonyms like `UserG|P`) all come from production. As of
2026-08-23 there are 399 entries.

**Two independent, differently-scoped consumers read this same array:**

| Consumer | What it does with `cachedSongs` |
| --- | --- |
| `LocalDanceStatsFileManager.GetStats()` (`m4dModels.Sandbox`) | Parses the file, then **discards** `cachedSongs` (replaces it with `[]`) before handing the JSON to `DanceStatsManager` — the per-dance top-songs cache starts empty every run |
| `SandboxServiceFactory.LoadCachedSongs()` (`m4dModels.Sandbox`) | Parses the file _independently_ and feeds every entry through `Song.Create(raw, dms)` — this is what actually seeds the sandbox's in-memory song database on startup (`m4d.Sandbox/Program.cs`, `SeedSongs`) |

So editing `cachedSongs` by hand (or by script) changes what songs exist in a freshly-started
sandbox instance, but has no effect on the dance/tag statistics shown in that same sandbox — those
come from `dances`/`groups`/`tagGroups`, computed once in production and frozen into the file.

**GUID formatting — braces.** `Song.ToString()` serializes via `SongId.ToString("B")`
(`m4dModels/Song.cs`, `Serialize(string[], string)`), which renders the GUID with braces:
`SongId={b8021347-433d-4607-99b5-33fe2184e655}`. This differs from the production index backup
(§12.2), which has no braces — see the comparison below.

### 12.2 Production Search Index Backup (`/Admin/IndexBackup`, `local/index-{yyyy}-{MM}-{dd}.txt`)

Generated by `AdminController.IndexBackup` → `SongIndex.BackupIndexStreamingAsync` (see
[index-backup-streaming.md](index-backup-streaming.md) for the streaming/pagination architecture).
**Not** committed to the repo — it's a manually-triggered export a developer downloads to `local/`
for one-off analysis or migration, like the 2026-08-23 ISRC backfill this section documents.

Despite living in `AdminController`'s `BackupDatabase`/`IndexBackup` neighborhood, this is a search
index export, not a SQL database dump — `BackupDatabase`'s own `songs` parameter has been dead code
(`// DBKILL:` — commented out) for some time, so a `backup-{yyyy}-{MM}-{dd}.txt` from that endpoint
contains **users, dances, tags, playlists, and searches, but no song data at all**. If you're
looking for production song data (ISRC, purchase IDs, tags, ratings, full edit history), the index
backup is the only file that has it — the SQL "database" backup does not.

**Format:** plain text, one song per line (103,909 lines as of 2026-08-23 — the corpus size cited
in §11.2). Each line is `Song.Serialize(id, properties)` (`m4dModels/Song.cs`, the **static**
overload) called with the Azure Search document's raw `SongId` field string and its properties
already run through `SongPropertyCompression.Decompress` — so every line is plain, uncompressed §1
wire format regardless of whether that row happened to be stored compressed in the index (§11).
Line ordering follows the backup's `(Modified desc, SongId desc)` key-set pagination, not any
song-relevant order.

**GUID formatting — no braces.** The `SongId` field on the Azure Search document is written from
`song.SongId.ToString()` with no format specifier (`SongIndex.cs`, `DocumentFromSong`) — the
default `"D"` format, no braces. A line looks like (tabs shown as spaces for readability, as in §1.1):

```text
SongId=6a85cd4e-2981-4017-b433-cfdacd443d0a  .Create=  User=RosarioAlcantar|P  Time=08/23/2026 09:07:48  Title=Con Paso Fino  ...
```

### 12.3 Reconciling the Two When Matching by SongId

Both files use the exact same per-song wire format (§1) and both round-trip through `Guid.TryParse`
without incident — `Song.TryParseId` (`m4dModels/Song.cs`) accepts either braced or unbraced input,
and `Song.BuildAlbumInfo` (`m4dModels/Song.cs`) keys albums/purchases by the **numeric** value of
their `:NN` index, so e.g. `Album:0` and `Album:00` denote the same slot regardless of digit
padding. Nothing breaks if the two are round-tripped through the app.

But a script comparing the two files textually — matching songs between a sandbox fixture (§12.1)
and a production export (§12.2) by `SongId`, as the ISRC backfill did — must normalize both: strip
`{}` and lowercase before comparing GUIDs, and compare `:NN` album/purchase indices as integers, not
as substrings. `local/scripts/backfill-test-isrc.js` has a working reference implementation of both
normalizations if a similar migration is needed again.
