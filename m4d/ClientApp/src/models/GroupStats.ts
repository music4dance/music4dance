...
export class GroupStats extends DanceGroup implements DanceStats {
  ...
  constructor(base: DanceGroup, dances: TypeStats[]) {
    ...
    this.songCount = this.dances.reduce((acc, dance) => acc + dance.songCount, 0);
    this.maxWeight = this.dances.reduce((acc, dance) => Math.max(acc + dance.maxWeight), 0);
    this.songTags = TagList.build(
      this.dances.reduce((acc: Tag[], dance) => TagList.concat(acc, dance.tags), []),
    ).summary!;
  }
