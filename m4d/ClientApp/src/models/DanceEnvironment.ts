...
export class DanceEnvironment {
  ...
  public rehydrate(): void {
    ...
    this.statsIdMap = new Map<string, DanceStats>(this.dances?.map((d) => [d.id, d]));
    this.tree?.forEach((g) =>
  ...
  public get flatStats(): DanceStats[] {
    if (!this.tree || !this.dances) {
      throw new Error("Attempted to call flatStats on an uninitialized DanceEnvironment");
    }
  ...
  public get flatDances(): NamedObject[] {
    if (!this.tree || !this.dances) {
      throw new Error("Attempted to call flatDances on an uninitialized DanceEnvironment");
    }
    return [...this.tree, ...this.dances];
  }
  ...
  public get groupedStats(): DanceStats[] {
    return this.tree!.sort((a, b) => a.name.localeCompare(b.name)).flatMap((group) => [
      group,
