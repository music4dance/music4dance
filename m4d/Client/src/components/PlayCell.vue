...
@Component({ components: { DanceVote, LikeModal, SongLikeButton, PlayModal } })
export default class PlayCell extends Mixins(AdminTools) {
  ...
  private get danceRating(): DanceRating {
    return this.song.findDanceRatingById(this.filter.danceQuery.danceList[0])!;
  }
  ...
  private get likeId(): string {
    return `like-${this.editor.songId}`;
  }

  private showPlayModal(): void {
