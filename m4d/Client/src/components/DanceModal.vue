<template>
    <b-modal :id="danceHandler.id" :header-bg-variant="tag.variant" header-text-variant="light" hide-footer>
        ...
        <b-list-group>
            ...
            <b-list-group-item :href="includeDance" variant="warning" v-if="hasFilter">
            ...
            <b-list-group-item 
                ...
                variant="info" 
                target="_blank">Help</b-list-group-item>
        </b-list-group>
        <div v-if="tags" style="margin-top:.5em">
            <tag-button v-for="tag in tags" :key="tag.key"
        ...
        <div>
            ...
            ></vote-button>
            <span style="padding-inline-start: 1em">I enjoy dancing <b>{{danceName}}</b> to {{this.title}}.</span>
            <div>The top song in the {{danceName}}} category has {{maxWeight}} votes.</div>
        </div>
    </b-modal>
</template>
...
export default class DanceModal extends Vue {
    ...
    private get includeDance(): string {
        return `${this.includeOnly}&filter=${this.danceHandler.filter!.encodedQuery}`;
    }
