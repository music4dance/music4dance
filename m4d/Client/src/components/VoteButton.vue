...
export default class VoteButton extends Vue {
    ...
    private get numberTip(): string {
        return `This song has ${this.danceRating.weight} votes.` +
    ...
    private get vote(): boolean | undefined {
        return TagList.build(this.song!.currentUserTags).voteFromTags(this.danceRating.positiveTag);
    }
    ...
    private removeVote(tag: Tag): void {
        this.song.currentUserTags = TagList.build(this.song.currentUserTags).remove(tag).tags;
    }
    ...
    private async submitVote(like?: boolean): Promise<void> {
        try {
            const response =  await axios.put(
                ...
                { like, dance: this.danceRating.danceId });
