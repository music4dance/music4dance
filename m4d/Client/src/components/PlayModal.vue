<template>
    <b-modal :id="song.songId" :title="title" ok-only ok-title="Close"
        @shown="onShown">
        <b-list-group>
            <b-list-group-item v-for="pi in purchaseInfo" :key="pi.songId"
                :href="pi.link" target="_blank"
            >
                <img width="32" height="32" :src="pi.logo">
                Available on {{ pi.name }}
            </b-list-group-item>
            <div v-if="song.sample">
                <audio :id="playerId" ref="player" :src="song.sample" controls
