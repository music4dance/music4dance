      ...
      <info-card :card="funCard">
        <div style="padding: .25em 1em">
          <div style="font-size:1.5em">
            <a href="/home/contribute"><b-icon-music-note-list></b-icon-music-note-list> Follow music4dance on Spotify</a>
          </div>
          <iframe src="https://open.spotify.com/follow/1/?uri=spotify:user:ebo1rk39vp51kkyjps45eobph&size=basic&theme=light&show-count=0"
            width="200" height="25" scrolling="no" frameborder="0" style="border:none; overflow:hidden; padding-left: 1em"
            allowtransparency="true"></iframe>
        </div>
        <div style="padding: .25em 1em">
          <div style="font-size:1.5em">
            <a href="/home/contribute"><b-icon-music-note-list></b-icon-music-note-list> Like music4dance on Facebook</a>
          </div>
          <div class="fb-like" data-href="https://www.facebook.com/music4dance.net" 
            data-width="" data-layout="standard" data-action="like" 
            data-size="small" data-share="true" style="padding-left: 1em">
          </div>
        </div>
      </info-card>
      <info-card v-for="card in cards" :key="card.title.link" :card="card"></info-card>
    </b-card-group>
    <b-row>
      <area-icon v-for="area in areas" :key="area.name" :area="area"></area-icon>
    </b-row>
    <b-row>
      <home-section name="Music" category="music" :features="musicFeatures" class="col-xl"></home-section>
      <home-section name="Tools" category="tools" :features="toolFeatures" class="col-xl"></home-section>
      <home-section name="Info" category="info" class="col-xl">
          <blog-feature-link v-for="entry in model.blogEntries" :key="entry.title" :entry="entry"
          ></blog-feature-link>
      </home-section>
    </b-row>
  </page>
...
export default class App extends Vue {
  ...
  private readonly areas: AreaInfo[] = [
      ...
      { name: 'Music' }, { name: 'Tools'}, { name: 'Info' },
    ];
  ...
  private readonly toolFeatures: FeatureInfo[] = [
    ...
    {
      title: 'Use our advanced search tool to find songs to dance to based one tempo, tags, dance style, and more.',
      type: 'tools',
  ...
  private get cards(): CardInfo[] {
    return  this.model.dances.map((d) => ({
      ...
      image: d.image,
      items: d.dances.map((dm) => ({ text: dm.title, link: this.danceLink(dm)})),
  ...
  private get funCard(): CardInfo {
    return {
      title: { text: 'Fun with Music for All Kinds of Dancers', link: 'https://music4dance.blog/' },
      image: 'fun',
