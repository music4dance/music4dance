...
describe('dance query description', () => {
    ...
    it ('should handle a single dance with inferred', () => {
        ...
        expect(dq.description).toEqual('Bachata songs (including inferred by tempo)');
    });

    it ('should handle a two dances, inclusive', () => {
        const dq = new DanceQuery('BCH,BOL');
        expect(dq.description).toEqual('songs danceable to any of Bachata or Bolero');
    });

    it ('should handle a two dances, exclusive', () => {
        const dq = new DanceQuery('AND,BCH,BOL');
        expect(dq.description).toEqual('songs danceable to all of Bachata and Bolero');
    });

    it ('should handle a two dances, inclusive, inferred', () => {
        const dq = new DanceQuery('OOX,BCH,BOL');
        expect(dq.description).toEqual('songs danceable to any of Bachata or Bolero (including inferred by tempo)');
    });

    it ('should handle a two dances, exclusive, inferred', () => {
        const dq = new DanceQuery('ADX,BCH,BOL');
        expect(dq.description).toEqual('songs danceable to all of Bachata and Bolero (including inferred by tempo)');
    });

    it ('should handle a multiple dances, inclusive', () => {
        const dq = new DanceQuery('BCH,BOL,BLU');
        expect(dq.description).toEqual('songs danceable to any of Bachata, Bolero or Blues');
    });

    it ('should handle a multiple dances, exclusive', () => {
        const dq = new DanceQuery('AND,BCH,BOL,BLU');
        expect(dq.description).toEqual('songs danceable to all of Bachata, Bolero and Blues');
    });

    it ('should handle a multiple dances, inclusive, inferred', () => {
        const dq = new DanceQuery('OOX,BCH,BOL,BLU');
        expect(dq.description).toEqual('songs danceable to any of Bachata, Bolero or Blues (including inferred by tempo)');
    });

    it ('should handle a multiple dances, exclusive, inferred', () => {
        const dq = new DanceQuery('ADX,BCH,BOL,BLU');
        expect(dq.description).toEqual('songs danceable to all of Bachata, Bolero and Blues (including inferred by tempo)');
    });
