...
describe('song property name tests', () => {
    ...
    it ('should fail on a non-track name', () => {
        ...
        expect(() => { const x = p?.index; }).toThrow(/Attempted to retrieve part.*/);
    });

    it ('should fail on a non-number track index', () => {
        const p = TypedJSON.parse({name: 'name:t:ss'}, SongProperty);

        expect(() => { const x = p?.index; }).toThrow(/Index must be a number.*/);
    });

    it ('should fail on a non-number track index', () => {
        const p = TypedJSON.parse({name: 'name:-1:ss'}, SongProperty);

        expect(() => { const x = p?.index; }).toThrow(/Index must be a postitive integer.*/);
    });
...
describe('song property value tests', () => {
    ...
    it ('should parse a float', () => {
        const p = TypedJSON.parse({name: 'Valence', value: '0.345'}, SongProperty);
    ...
    it ('should parse a date ', () => {
        const p = TypedJSON.parse({name: 'Created', value: '08/29/2017 20:32:01'}, SongProperty);
