...
describe('dance rating delta serialize/deserialize', () => {
    ...
    it ('should throw on falsey', () => {
        expect(() => {DanceRatingDelta.fromString(''); }).toThrowError('value must not be falsey');
    });

    it ('should throw on misformatted string', () => {
        const error = 'value must conform to {dancid}(+|-){count}';
        expect(() => {DanceRatingDelta.fromString('LTN'); }).toThrowError(error);
        expect(() => {DanceRatingDelta.fromString('LTN+'); }).toThrowError(error);
        expect(() => {DanceRatingDelta.fromString('LTN!34'); }).toThrowError(error);
        expect(() => {DanceRatingDelta.fromString('LTN+SWG'); }).toThrowError(error);
    });
