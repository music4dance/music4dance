...
@jsonObject export class TempoRange {
    ...
    public bpm(numerator: number): string {
        return this.formatTempo(this.min * numerator) +
            ((this.min === this.max) ? '' : '-' + this.formatTempo(this.max * numerator));
    }
    ...
    public combine(that: TempoRange): TempoRange {
        return new TempoRange(
            Math.min(this.min, that.min), Math.max(this.max, that.max),
        );
    }
...
@jsonObject export class DanceException extends DanceObject {
    ...
    public matchesFilter(filter: string): boolean {
        ...
        if (parts[1] === '2' && (this.level === 'Silver,Gold' || this.competitor === 'Professional,Amateur')) {
            return true;
        }
        return false;
    }
...
@jsonObject export class DanceType extends DanceObject {
    ...
    public match(filter: DanceFilter): boolean {
        return filter.types.find((t) => kebabToWords(t) === this.groupName) != null &&
            filter.meters.find((m) => m === `${this.meter.numerator}-${this.meter.denominator}`) != null &&
            filter.styles.find((s) => this.instances.find((inst) => kebabToWords(s) === inst.style)) != null;
    }
