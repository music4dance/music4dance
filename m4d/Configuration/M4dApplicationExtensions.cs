...
{
    /// <summary>
    /// When false, skips the UseSqlServer DbContext registration/migration and the
    /// post-migration seed-data call in UseM4dPipeline - the caller owns DbContext
    /// registration and seeding entirely.
    /// </summary>
    public bool ConfigureDatabase { get; init; } = true;

    /// <summary>
    /// When false, skips the Azure Search client wiring and the ISearchServiceManager /
    /// IDanceStatsManager singleton registrations - the caller supplies its own.
    /// </summary>
    public bool ConfigureSearch { get; init; } = true;
}
...
/// </summary>
public static class M4dApplicationExtensions
{
    public static WebApplicationBuilder AddM4dApplication(
        this WebApplicationBuilder builder,
        string connectionString,
        M4dApplicationOptions appOptions = null)
    {
        appOptions ??= new M4dApplicationOptions();
        ...
        if (appOptions.ConfigureDatabase)
        {
    ...
    public static async Task<WebApplication> UseM4dPipeline(
        ...
        M4dApplicationOptions appOptions = null)
    {
        ...
        if (!isDevelopment)
        {
            ...
            _ = app.Use(async (context, next) =>
            {
                ...
                {
                    var logger = context.RequestServices.GetRequiredService<ILogger<global::Program>>();
