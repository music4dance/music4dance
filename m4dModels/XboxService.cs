    ...
    public class XboxService : MusicService
    {
        public XboxService() :
            base(ServiceType.XBox,
                ...
                "https://music.xboxlive.com/1/content/{0}/lookup")
        {
        ...
        public override IList<ServiceTrack> ParseSearchResults(dynamic results)
        {
            ...
            {
                ret.Add(InternalParseTrackResults(track));
            }
        ...
        private static ServiceTrack InternalParseTrackResults(dynamic track)
        {
            ...
            if (track.OtherIds != null)
            {
                try
                {
                    altId = track.OtherIds.music_amg;
                }
            ...
            return new ServiceTrack
            {
                ...
                Name = track.Name,
                AltId = altId,
                Artist = track.Artists?.Length > 0 ? track.Artists[0].Artist.Name : null,
                Album = track.Album.Name,
                ImageUrl = track.ImageUrl,
                //                    Link = track.Link + "?action=play&target=app",
                ReleaseDate = track.ReleaseDate,
                Genre = track.Genres?.Count > 0 ? track.Genres[0] : null,
                Duration = duration,
